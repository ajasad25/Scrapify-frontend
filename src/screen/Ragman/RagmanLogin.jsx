  ...
  const panAnim = useRef(new Animated.Value(0)).current;
    ...
    const handleLogin = async () => {
        ...
        const data = {
          email,
          password,
          role: 'RAGMAN' // Add role parameter for Ragman login
        };
        ...
        try {
          ...
          if (response.ok) {
            ...
            // Try different possible field names for user ID - check result.user.id first!
          const userId = result.user?.id || result.userId || result.id || result.user_id || result.ragmanId;
          ...
          console.log("DEBUG: Full user object:", result.user);
            ...
            await AsyncStorage.setItem("@user_id", String(userId)); // <-- Save ragman user id
...
/*
  ...
  const handleLogin = async () => {
    ...
    // Dummy login for testing UI without backend
    if (email === "ragman@gmail.com" && password === "ragman123") {
      // Commented out network-related and context parts
      ...
      >
        ...
        >
          ...
          <Text style={styles.welcomeSub}>
            Welcome <Text style={styles.boldText}>Ragman!</Text>
          </Text>
